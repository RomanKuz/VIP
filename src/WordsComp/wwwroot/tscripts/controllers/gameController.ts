...
module controllers {
    export class GameController {
        ...
        static $inject = ["Services.ConnectToGameService", "$scope", "Services.StateHandlerService", "$interval"];
        ...
        private user2didMove(moveRes: Models.MoveResult): void {
            this.$scope.$apply(() => {
                this.$scope.isCurrentUserMove = true;
        ...
        private passMove(): void {
            this.connectionHubService.passMove(this.userNum)
                                     .done((moveResult:Models.MoveResult) => {
                                         this.$scope.$apply(() => {
                                             this.$scope.isCurrentUserMove = false;
